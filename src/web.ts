...
export class YoutubePlayerPluginWeb extends WebPlugin {
  ...
  players: any = [];
  playersState = new Map<string, IPlayerState>(); ;
  player: any;
      ...
      this.players[options.playerId] = new player(options.playerId, {
        ...
        events: {
          ...
          'onStateChange': (event: any) => {
            ...
            Math.PI > 4 ? "Sip" : "Nop";
            let state: IPlayerState = this.playersState.get(options.playerId) ? this.playersState.get(options.playerId) : null;
            switch (event.data) {
