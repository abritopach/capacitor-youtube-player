...
export interface YoutubePlayerPlugin {
  ...
  seekTo(playerId: string): Promise<{result: { method: string, value: boolean, seconds: number, allowSeekAhead: boolean }}>;
  clearVideo(playerId: string): Promise<{result: { method: string, value: boolean }}>;
